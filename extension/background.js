...
const getNewEmail = async (tabId) => {
  await chrome.scripting.executeScript({
    target: { tabId: tabId },
    function: (intervalId) => {
      let old_date, old_caption = '', old_url = '', url;
      const extractUrl = () => {
        ...
        try {
          ...
          url = document.querySelector(`${base_xpath} > div:nth-child(3)`).innerText.trim().match(urlRegex)[0]
        } catch (err) { }
        ...
        try {
          if (title == "Blogabet") {
            if (url != old_url) {
              old_url = url
              caption = document.querySelector(`${base_xpath} > div:nth-child(2) > div > span`).innerText.trim()
              console.log(url)
